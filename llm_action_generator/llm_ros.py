...
class LLAMARobotExecutor(Node):
    def __init__(self):
        ...
        self.declare_parameter('cup_pose', [0.4, 0.2, 0.3])
        self.declare_parameter('bottle_pose', [0.3, -0.3, 0.4])
        self.declare_parameter('hover_height', 0.06)

        self.cup_pos = tuple(self.get_parameter('cup_pose').get_parameter_value().double_array_value)
        self.bottle_pos = tuple(self.get_parameter('bottle_pose').get_parameter_value().double_array_value)
        self.hover_height = self.get_parameter('hover_height').get_parameter_value().double_value

        self.get_logger().info(f"Cup: {self.cup_pos}, Bottle: {self.bottle_pos}, Hover: {self.hover_height}")
